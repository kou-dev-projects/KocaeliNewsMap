  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        status: {
          enum: ["running", "success", "partial", "failed", "cancelled", "timeout"]
        },
...
ensureIndex("crawl_sessions", { source_id: 1, started_at: -1 }, { name: "source_started_at" });
ensureIndex("crawl_sessions", { status: 1, started_at: -1 }, { name: "status_started_at" });
ensureIndex("crawl_sessions", { created_at: 1 }, { name: "ttl_90days", expireAfterSeconds: 7776000 });
...
ensureIndex("raw_documents", { source_id: 1, scraped_at: -1 }, { name: "source_scraped_at" });
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        geocode_status: { enum: ["pending", "resolved", "failed", "approximate", "not_needed"] },
        ...
        duplicate_reason: { bsonType: "string" },
        pipeline_status: {
          enum: ["ingested", "normalized", "classified", "geocoded", "embedded", "clustered", "served", "failed"]
        },
...
ensureIndex("source_records", { raw_document_id: 1 }, { name: "raw_document_id_unique", unique: true });
ensureIndex("source_records", { canonical_url: 1 }, { name: "source_record_url" });
ensureIndex("source_records", { source_id: 1, published_at: -1 }, { name: "source_published_at" });
ensureIndex("source_records", { category_predicted: 1, district_predicted: 1, published_at: -1 }, { name: "category_district_published_at" });
ensureIndex("source_records", { geocode_point: "2dsphere" }, { name: "geocode_point_2dsphere", sparse: true });
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        status: { enum: ["open", "resolved", "monitoring", "archived", "review"] },
...
ensureIndex("events", { event_type: 1, latest_published_at: -1 }, { name: "event_type_latest_published_at" });
ensureIndex("events", { district: 1, latest_published_at: -1 }, { name: "district_latest_published_at" });
ensureIndex("events", { is_active: 1, latest_published_at: -1 }, { name: "active_latest_published_at" });
ensureIndex("events", { centroid: "2dsphere" }, { name: "centroid_2dsphere", sparse: true });
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        match_score: { bsonType: ["double", "int", "decimal"], minimum: 0, maximum: 1 },
        match_reason: { enum: ["hash_exact", "embedding_90_plus", "title_similarity", "manual_review"] },
...
ensureIndex("event_records", { source_record_id: 1 }, { name: "source_record_id_unique", unique: true });
ensureIndex("event_records", { event_id: 1 }, { name: "event_id" });
ensureIndex("event_records", { event_id: 1, is_primary: 1 }, {
...
ensureCollection("geocoding_cache", {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      additionalProperties: false,
      required: ["_id", "normalized_query", "provider", "status", "result_quality", "created_at", "expires_at"],
      properties: {
        _id: { bsonType: "string" },
        normalized_query: { bsonType: "string" },
        provider: { enum: ["nominatim", "pelias", "manual"] },
        status: { enum: ["hit", "miss", "failed", "ambiguous"] },
        result_quality: { enum: ["exact", "approximate", "district", "city", "none"] },
        result_point: {
          bsonType: ["object", "null"],
          additionalProperties: false,
          required: ["type", "coordinates"],
        ...
        confidence: { bsonType: ["double", "int", "decimal"], minimum: 0, maximum: 1 },
...
ensureIndex("geocoding_cache", { expires_at: 1 }, { name: "ttl_expires_at", expireAfterSeconds: 0 });
ensureIndex("geocoding_cache", { result_point: "2dsphere" }, { name: "result_point_2dsphere", sparse: true });
ensureIndex("geocoding_cache", { provider: 1, normalized_query: 1 }, { name: "provider_query" });
...
ensureCollection("review_queue", {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      additionalProperties: false,
      required: ["entity_type", "entity_object_id", "reason", "status", "created_at"],
      properties: {
        _id: { bsonType: "objectId" },
...
ensureIndex("review_queue", { status: 1, created_at: -1 }, { name: "status_created_at" });

// Seed mandatory sources
[
  { domain: "cagdaskocaeli.com.tr", display_name: "Çağdaş Kocaeli", base_url: "https://www.cagdaskocaeli.com.tr", scraper_type: "static" },
  { domain: "ozgurkocaeli.com.tr", display_name: "Özgür Kocaeli", base_url: "https://www.ozgurkocaeli.com.tr", scraper_type: "static" },
  { domain: "seskocaeli.com", display_name: "Ses Kocaeli", base_url: "https://www.seskocaeli.com", scraper_type: "dynamic" },
  { domain: "yenikocaeli.com", display_name: "Yeni Kocaeli", base_url: "https://www.yenikocaeli.com", scraper_type: "static" },
  { domain: "bizimyaka.com", display_name: "Bizim Yaka Kocaeli", base_url: "https://www.bizimyaka.com", scraper_type: "dynamic" }
...
// Startup check
const activeDomains = db.sources.find({ active: true }, { domain: 1, _id: 0 }).toArray().map(function (x) { return x.domain; });
const missing = SOURCE_WHITELIST.filter(function (d) { return activeDomains.indexOf(d) === -1; });
if (missing.length > 0) {
  throw new Error("Missing mandatory domains: " + missing.join(", "));
}

print("Sprint 1 clean schema initialized successfully.");